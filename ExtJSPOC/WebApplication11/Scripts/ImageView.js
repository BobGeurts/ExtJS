...
Ext.define('Ext.org.ImageView', {
    ...
    tpl: [
        '<tpl for=".">',
            '<div class="thumb-wrap">',
                '<div class="thumb">',
                    (!Ext.isIE6? '<img src="ext/examples/view/chooser/icons/{thumb}" />' : 
                    '<div style="width:76px;height:76px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'ext/examples/view/chooser/icons/{thumb}\')"></div>'),
                '</div>',
                '<span>{name}</span>',
            '</div>',
        '</tpl>'
    ...
    initComponent: function() {
        this.store = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                url : 'ext/examples/view/chooser/icons.json',
                reader: {
                    type: 'json',
                    root: ''
                }
        ...
        this.mixins.draggable.init(this, {
            ...
            ghostTpl: [
                '<tpl for=".">',
                    '<img src="ext/examples/view/chooser/icons/{thumb}" />',
